...
run_get(){
  URL=$1
  NAME=$2
  RESP=$(curl -sS -w "\n%{http_code}" -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Accept: application/json' "$URL")
...
FIRST_PARTNER_ID=$(curl -sS -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Accept: application/json' http://localhost:8000/api/partners | php -r '$s=stream_get_contents(STDIN); $d=json_decode($s,true); if(!$d) exit; if(isset($d["data"][0]["id"])) echo $d["data"][0]["id"]; elseif(isset($d[0]["id"])) echo $d[0]["id"];')
...
if [ -z "$FIRST_PARTNER_ID" ]; then
  ...
  CREATE_PARTNER=$(curl -sS -w "\n%{http_code}" -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Accept: application/json' -H 'Content-Type: application/json' -X POST http://localhost:8000/api/partners -d '{"name":"Auto Partner"}')
...
if [ -n "$FIRST_PARTNER_ID" ]; then
  CREATE_RESP=$(curl -sS -w "\n%{http_code}" -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Accept: application/json' -H 'Content-Type: application/json' -X POST http://localhost:8000/api/invoices -d "{\"partner_id\": \"$FIRST_PARTNER_ID\", \"lines\": [{\"description\": \"Auto invoice\", \"quantity\":1, \"unit_price\": 10.0}]}" )
  ...
  if [ -n "$INVOICE_ID" ]; then
    PAY_RESP=$(curl -sS -w "\n%{http_code}" -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Accept: application/json' -H 'Content-Type: application/json' -X POST http://localhost:8000/api/invoices/$INVOICE_ID/payments -d "{\"invoice_id\": \"$INVOICE_ID\", \"amount\":10.0, \"method\":\"card\"}")
