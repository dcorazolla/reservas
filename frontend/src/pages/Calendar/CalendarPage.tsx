        ...
        <div className="calendar-scroll-wrapper">
          <div className="page-header">
            <div className="calendar-controls-row">
              ...
              <label className="control-label--ml8">
                Mês:
              ...
              <label className="control-label--ml12">
                Dias:
                <input type="number" value={String(days)} min={7} max={90} onChange={(e)=>setDays(Number(e.target.value))} className="input-days" />
              </label>
            </div>
          ...
          {loading ? (
            <div className="calendar-wrapper">
              <table className="calendar-table">
                <thead>
                  <tr>
                    <th className="room-col"><Skeleton variant="text" width={140} height={20} /></th>
                    {Array.from({ length: 7 }).map((_, i) => (
                      <th key={i}><Skeleton variant="text" width={100} height={20} /></th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {Array.from({ length: 6 }).map((_, r) => (
                    <tr key={r}>
                      <td className="room-col"><Skeleton variant="text" width={120} height={16} /></td>
                      {Array.from({ length: 7 }).map((__, c) => (
                        <td key={c}><div className="skeleton-cell"><Skeleton variant="rect" height={36} /></div></td>
                      ))}
                    </tr>
                  ))}
                </tbody>
