...
import { listProducts } from "../../api/minibar";
import { MinibarPanel } from "../Minibar";
...
export default function ReservationModal({
  ...
  const [showMinibarPanel, setShowMinibarPanel] = useState<boolean>(false);
  const [minibarProducts, setMinibarProducts] = useState<any[]>([]);
  const [localConsumptions, setLocalConsumptions] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      try {
        ...
        if (!mounted) return;
        setMinibarProducts(prods || []);
      } catch (e) {
        console.warn('Falha ao carregar produtos do frigobar', e);
        if (mounted) setMinibarProducts([]);
      }
    }
    if (showMinibarPanel) loadProducts();
  ...
  return (
    <Modal open={true} title={reservation ? "Editar Reserva" : "Nova Reserva"} titleId="reservation-title" onClose={onClose} closeOnBackdrop={false} className="modal--wide">
      <div className="form reservation-modal" aria-busy={initialLoading || calcLoading}>
        ...
        {showMinibarPanel && (
          <div className="minibar-quick-panel-highlight">
            {minibarProducts.length === 0 ? (
              <div>Carregando frigobar…</div>
            ) : (
              <MinibarPanel products={minibarProducts} onProductsChange={setMinibarProducts} initialReservationId={reservation?.id ?? null} onConsumptionCreated={(c) => setLocalConsumptions((s) => [c, ...s])} />
            )}
          </div>
        )}
        ...
        <div className="modal-status-row">
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {editing && (
              <>
                <button type="button" className={`status-btn status-${status} secondary`} onClick={() => setShowMinibarPanel(true)}>Frigobar</button>
                <button type="button" className="secondary" onClick={() => setShowMinibarPanel(v => !v)} aria-pressed={showMinibarPanel} title="Alternar exibição do frigobar">{showMinibarPanel ? 'Ocultar Frigobar' : 'Mostrar Frigobar'}</button>
              </>
            )}
        ...
        <div className="form-group">
          {/* Quick minibar panel toggle area: shows a small embedded minibar list when toggled */}
          {showMinibarPanel && (
            <div className="minibar-quick-panel" style={{ marginBottom: 12 }}>
              {minibarProducts.length === 0 ? (
                <div>Carregando frigobar…</div>
              ) : (
                <MinibarPanel products={minibarProducts} onProductsChange={setMinibarProducts} initialReservationId={reservation?.id ?? null} onConsumptionCreated={(c) => setLocalConsumptions((s) => [c, ...s])} />
