...
test('sets endDate to startDate + 1 day when opened from calendar', async () => {
  render(
    <ReservationModal roomId={null} date={'2026-02-14'} reservation={null} onClose={() => {}} />
  );
  const end = await screen.findByLabelText(/Saída/i);
  expect((end as HTMLInputElement).value).toBe('2026-02-15');
});
