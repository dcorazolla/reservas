...
// Automatically wrap all renders with Chakra provider so tests that render
// components using Chakra do not need to remember to wrap each render.
// We mock `@testing-library/react`'s `render` export to inject our provider.
import React from 'react'
import AppChakraProvider from './@BKP/src/design-system/ChakraProvider'

// Use an async factory to import the real testing library and override `render`.
vi.mock('@testing-library/react', async () => {
	const rtl = await vi.importActual('@testing-library/react')
	const originalRender = rtl.render
	return {
		...rtl,
		render: (ui: any, options?: any) => originalRender(React.createElement(AppChakraProvider, null, ui), options),
	}
})
